...
warning off
include "C:/Desktop/universe/action swf/actionswf/dev/import.h"
warning on
include "C:/Desktop/universe/action swf/actionswf/include/prog.h"

include "C:/Desktop/universe/action swf/files/tool.s"

                                             #oricum -Infinity<0
#nu conteaza: left_player_filter-- si right ajung la -Infinity dar ar trebui mult in jos
...
const matchnumber=3
const all_requirements=matchnumber*20

const brd_off=5
#asta e 2*brd_off si mai jos e direct 2*pl_container.table.brd_off
const scadere_dreapta=2*brd_off
function add_combatant(sd y,ss point,sd font,sd mover_pre)
    sd sprite
    sd presprite
    ...
    const fnt_sz=15
        ...
        sd txt;setcall txt dyn_text_color(txt_w,"mover_name",font,(fnt_sz),0xff0000ff);call swf_sprite_placeobject(presprite,txt,0)
    #
    const text_part_size=fnt_sz+10
    sd rect_w;set rect_w competitor_wd#
    sd rect_h=text_part_size;addcall rect_h draw_competitor(presprite,point,(text_part_size))
    #
    ...
    call action_sprite(presprite,"
        ...
        function redraw_brd(){
            ...
            brd.lineTo(rect_w+brd_off,text_part_size-brd_off);brd.lineTo(rect_w+brd_off,rect_h+brd_off);
            brd.lineTo(0-brd_off,rect_h+brd_off);brd.lineTo(0-brd_off,text_part_size-brd_off);
        }
        redraw_brd()
        ")
    #call action_sprite(presprite,"if(_root['episode'].can_start==1)_parent.mover_start()")
    call action_sprite(presprite,"_parent.mover_start()")
...
const points_x_add=11
const Y_add=20+5
#height
function draw_competitor(sd presprite,ss point,sd y)
    sd competitor_wd
    setcall competitor_wd competitor_w()
    sd max_x;set max_x competitor_wd#;sub max_x (scadere_dreapta)
    chars f#300
    str data^f
    sd i=0
    sd points_x
    ...
    while 1==1
        ...
        while points_x<max_x
            call actionf_sprite(presprite,data,"var nm='%s';var inst=nm+%u;points.attachMovie(nm,inst,points.getNextHighestDepth());points[inst]._x=%u;points[inst]._y=%u",point,i,points_x,points_y)
            add points_x (points_x_add)
...
str env_path="D:/universe/7/env.dbl"
...
#call sprintf(ac,"D:/universe/oapp/%s.swf",game_name)
call swf_new("../../test/ep7.swf",env_w,env_h,0x212121,(fps))

call action("
    _root['episode']=this;
    var com=_root['shared_level'];
    if(com.left_arrow==undefined)com.left_arrow=Key.LEFT;
    if(com.right_arrow==undefined)com.right_arrow=Key.RIGHT;
    if(com.down_arrow==undefined)com.down_arrow=Key.DOWN;
")
...
call action("attachMovie('env','env',getNextHighestDepth())")
#<env

#cubes>
str path="D:/universe/7/cubes/%s.dbl"
chars cb="blue_planet";chars *="galaxy";chars *="geyser";chars *="green_planet";chars *="ice_planet";chars *="jupiter";chars *="mountaines"
chars *="probe";chars *="red_planet";chars *="rocket";chars *="saturn";chars *="solar-system";chars *="sun";chars *="volcano"
chars *=0
ss cubes_names^cb
...
sd j=0;while j<2
    ...
    call action_sprite(presprite,"var ancestor=_parent._parent")
        #this for having vars attached before the next functions, at player at comp with max_thinking=0 required
    call swf_sprite_showframe(presprite)
    sd i=0
    while i<(fps)
        if j==0
...
call action_sprite(mover_pre,"var com=_root['shared_level']")
...
sub y_off 20
call add_combatant(y_off,p,deffont,mover_pre)
...
#var half_fps=%u;       ,(fps/2)
call actionf_sprite(mover_pre,ac,"var filter_limit=%u",(fps/30))
...
grid._y=(_root['episode'].env._height-grid_h)/2;
...
function cube_in(){
    ...
    var pos=random(_root['episode'].cubes.length);

/*
    if(_name=='Player'){pos=_root['episode'].debugging_player.splice(0,1);pos=pos[0];}
    else{pos=_root['episode'].debugging_comp.splice(0,1);pos=pos[0];}
*/

    var cube_rand=_root['episode'].cubes[pos];var inst=cube_rand+simple_counter;simple_counter++;
    ...
    if(_name=='Player'){
        //_root['episode']._text.message=_root['episode'].score;
        cube.onKeyDown=move_cube;
        Key.addListener(cube);
        left_player_filter=-1;
        right_player_filter=-1;
    }
    cube.attachMovie('vars_'+cube_rand,'vars',cube.getNextHighestDepth());
}
//am filtrat aicea
var left_player_filter;
var right_player_filter;
function move_cube(){
    if(Key.getCode()==com.left_arrow){
        if(left_player_filter<0)move_cube_left();
        else{left_filter();if(left_filter_value==0)move_cube_left();}
        left_player_filter=1;
    }
    else if(Key.getCode()==com.right_arrow){
        if(right_player_filter<0)move_cube_right();
        else{right_filter();if(right_filter_value==0)move_cube_right();}
        right_player_filter=1;
    }
    else if(Key.getCode()==com.down_arrow)can_go_down=1;
}
//aicea e 1 filter limit ca daca ar fi 0 ar fi nasol
var left_filter_value=0;
function left_filter(){
    left_filter_value++;
    if(left_filter_value>filter_limit)left_filter_value=0;
}
var right_filter_value=0;
function right_filter(){
    right_filter_value++;
    if(right_filter_value>filter_limit)right_filter_value=0;
}
var down_filter_value=0;
function down_filter(entry){
    down_filter_value++;
    if(down_filter_value>filter_limit){move_cube_y(entry);down_filter_value=0;}
}
//
function move_cube_y(entry){
    var cube_at_ground=false;
    var row=(entry._y+yOff_3d)/cell+1;var col;
    if(row==grid_height)cube_at_ground=true;
    else{
        col=(entry._x+xOff_3d)/cell;
...
function timer_end(){
    ...
    if(done_points>=all_points){
        got_all_points=1;
        if(_name=='Player')_root.end_scenario();
        else{
            var pl_container=_root['episode'].Player;
            pl_container.new_max_points=pl_container.all_points;
            //
            var pl_table_current_x=0;
            var pl_table_current_y=pl_container.table.rect_h-pl_container.table.points._y;
            pl_container.table.rect_h+=_root['episode'].table_add_value_y;
            pl_container.table.redraw_brd();
            //
            while(pl_table_current_x<(pl_container.table.rect_w-(2*pl_container.table.brd_off))){
                var pl_mv=pl_container.table.points.attachMovie('point','point'+pl_container.new_max_points,pl_container.table.points.getNextHighestDepth());
                pl_mv._x=pl_table_current_x;pl_mv._y=pl_table_current_y;
                //
                pl_container.new_max_points++;
                pl_table_current_x+=_root['episode'].table_add_value_x;
            }
            pl_container.all_points=pl_container.new_max_points;
        }
    }
...
function player_cube_frame(entry){
    left_player_filter--;right_player_filter--;
    if(can_go_down!=0)down_filter(entry);
}
function comp_cube_frame(entry){
    if(can_go_down>=1)down_filter(entry);
    else if(comp_thinking>=0){if(comp_thinking==0)computer_set();comp_thinking--;}
    else if(right_frames!=0){right_filter();if(right_filter_value==0){right_frames--;move_cube_right();}}
    else can_go_down=1;
}
...
call actionf(ac,"var table_add_value_x=%u;var table_add_value_y=%u",(points_x_add),(Y_add))

#sd texter_y
#set texter_y env_h
#sub texter_y 40
#call default_text_ex((0xff0101FF),deffont,texter_y,40)
#call action("_text.message='qwer'")
#include "debugging.s"
